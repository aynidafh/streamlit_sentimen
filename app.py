...
[data-testid="stSidebar"] h1 { color: #F8FAFC !important; font-size: 1rem !important; font-weight: 600 !important; letter-spacing: 0.05em; text-transform: uppercase; }
...
# ─── ANALISIS DATA ─────────────────────────────────────────────────────────────
    ...
    if not data_loaded:
        ...
        if 'jumlah_karakter' in df_raw.columns and 'jumlah_kata' in df_raw.columns:
            c3, c4 = st.columns(2)
            with c3:
                st.markdown("#### Distribusi Jumlah Karakter per Tweet")
                fig, ax = styled_plot((6, 4))
            ...
            with c4:
                st.markdown("#### Distribusi Jumlah Kata per Tweet")
                fig, ax = styled_plot((6, 4))
            ...
            stats = df_raw[['jumlah_karakter', 'jumlah_kata']].describe().round(2)
